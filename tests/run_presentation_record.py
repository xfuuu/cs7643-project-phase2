...
-----
  full       Steps 1–10 from the Layer-2 regression route (no mid-game save/load),
             then continues through plot steps 11–16 until ``game.py`` exits on the
             resolution beat. One continuous subprocess session → one coherent terminal log.
...
EXTRA_FULL_PLAN: list[Check] = [
    Check(
        "[FULL 11] Step 9 — reach Study & analyse the bloodless stab wound",
        [
            "go guest wing",
            "go main corridor",
            ...
            "go library",
            "go study",
            "examine the stab wound and the absence of blood spray on the body",
        ],
    ...
    Check(
        ...
        [
            ...
            "go main corridor",
            "go guest wing",
            "ask Dr Vance about the paper he tried to conceal in his medical bag",
        ],
    ...
    Check(
        ...
        [
            ...
            "go main entrance hall",
            "go drawing room",
            "ask Eleanor about where she was while her husband was questioned",
        ],
        expected_title="The Gardener's Secret",
        critical=False,
    ),
    Check(
        "[FULL 14] Step 12 — Conservatory / monkshood glove",
        [
            "go conservatory",
            "search the waste bin for gardening gloves or plant residue",
        ],
        expected_title="The Beaded Handbag",
        critical=False,
    ),
    Check(
        "[FULL 15] Step 13 — Main Entrance Hall / handbag petal",
        [
            "go drawing room",
            "go main entrance hall",
            "examine the guests' cloaks and Eleanor Vance's handbag",
        ],
        expected_title="The Midnight Reckoning",
        critical=False,
    ),
    Check(
        "[FULL 16] Step 14 — Library / midnight reckoning analysis",
        [
            "go drawing room",
            "go library",
            "review how the poison timeline implicates Eleanor using all gathered clues",
        ],
        expected_title="The Final Mask Removed",
        critical=False,
    ),
    Check(
        "[FULL 17] Step 15 — Study / final confrontation (may trigger exit)",
        [
            "go study",
            "accuse Eleanor Vance of poisoning Sir Alistair with monkshood before the assembled suspects",
        ],
        expected_title=None,
        critical=False,
    ),
...
def _game_args(llm_log: Path, llm_full: bool) -> list[str]:
...
def run_full(api_key: str, llm_log: Path, llm_full: bool) -> int:
    """Layer-2 checks [1–10] without save/load, then EXTRA_FULL_PLAN → exit."""
...
def main() -> None:
    ...
    ap.add_argument("--mode", choices=("full", "exception"), required=True)
    ap.add_argument("--api-key", default=os.environ.get("GEMINI_API_KEY", ""))
    ap.add_argument(
        "--terminal-log",
        required=True,
        type=Path,
        help="Append-free path; parent dirs are created.",
    )
    ap.add_argument(
        "--llm-log",
        required=True,
        type=Path,
        help="JSONL path for LLM calls (truncated at start of each run).",
    )
    ap.add_argument(
        "--llm-log-full",
        action="store_true",
    ...
    with args.terminal_log.open("w", encoding="utf-8") as tf:
        ...
        try:
            ...
            if args.mode == "full":
                rc = run_full(args.api_key, args.llm_log, args.llm_log_full)
            else:
                rc = run_exception(
                    args.api_key,
                    args.llm_log,
